
{{< include _math.qmd >}}

# O produto externo {#sec-prod-ext}


## Vetores em $\reais^2$ {#sec-vetores-r2}

* Por enquanto, vamos trabalhar no espaço vetorial $\reais^2$.

* Os elementos de $\reais^2$ são vetores com duas coordenadas; por exemplo:

  $$
  \begin{array}{l}
    \vetor v = (-1, 3)\\
    \vetor w = \left( \frac12, \frac{\sqrt{2}}{2} \right)
  \end{array}
  $$


  ::: {.callout-warning}
  
  ### Notação: vetores em negrito
  
  Você deve estar acostumado a escrever nomes de vetores como $\vec v$, $\vec w$ etc.
  
  Neste livro, como na maioria dos livros sobre álgebra geométrica, nomes de vetores serão escritos em negrito: $\vetor v$, $\vetor w$.
  
  :::


* Usando a base canônica de $\reais^2$, com $\ve1 = (1, 0)$ e $\ve2 = (0, 1)$, os vetores do exemplo acima podem ser escritos como

  $$
  \begin{array}{l}
    \vetor v = -1\ve1 + 3\ve2\\
    \vetor w = \frac12 \ve1 + \frac{\sqrt{2}}{2}\ve2
  \end{array}
  $$

* Tecnicamente, estamos escrevendo cada vetor como uma [combinação linear]{.hl} dos vetores da base $\{ \ve1, \ve2 \}$. 


* Para lembrar que estamos trabalhando com $\ve1$ e com $\ve2$, vamos rotular os eixos $x$ e $y$ dos nossos gráficos com os nomes destes dois vetores, como na @fig-e1-e2.

* Mas você deve se lembrar que $\ve1$ e $\ve2$ representam os dois vetores unitários da figura, e não os eixos orientados (que são infinitos).

  ![Vetores da base canônica e eixos](figures/vetores-base.png){width=70% fig-alt="Eixos e vetores da base canônica." #fig-e1-e2}

  ::: {.callout-warning}
  
  ### Notação: vetores como combinações lineares dos vetores da base
  
  Como na maioria dos livros sobre álgebra geométrica, em vez de escrevermos
  $$
  \vetor v = (x, y)
  $$
  vamos escrever
  $$
  \vetor v = x\ve1 + y\ve2
  $$
  
  Se uma das coordenadas for zero, podemos omitir o vetor da base correspondente. Por exemplo, vamos escrever o vetor
  $$
  \vetor u = (0, 3)
  $$
  como
  $$
  \vetor u = 3\ve2
  $$
  
  :::


* []{#topicos-vetores} Para acompanhar o restante deste capítulo, você deve revisar os seguintes tópicos sobre vetores, especialmente em $\reais^2$ e em $\reais^3$:

  * Adição de vetores,
  * Multiplicação de vetor por escalar (nossos escalares vão ser números reais),
  * Vetor nulo,
  * Vetor inverso (para a adição),
  * Dependência e independência linear,
  * Módulo (norma) de um vetor,
  * Produto vetorial,
  * Subespaços vetoriais.
  

## Retas em $\reais^2$

* Por enquanto, só temos vetores.

* Cada vetor (diferente de $\vetor 0$, o vetor nulo) indica uma direção.

* Mas apenas uma direção não basta para definir uma reta. Por exemplo, todas as retas da @fig-retas têm a mesma direção: a direção dada pelo vetor $\vetor v = \ve1 + 2\ve2$.

  ![Retas e vetores](figures/reta-homogenea.png){width=80% fig-alt="Retas e vetores paralelos." #fig-retas}

* Vamos combinar que [todas as nossas retas de interesse passam pela origem]{.hl} --- ou seja, pelo ponto $O=(0,0)$. 

* Fazendo isto, cada vetor determina uma única reta.

* Chamamos as retas que passam pela origem de [retas homogêneas]{.hl}. Na @fig-retas, só há uma reta homogênea (a reta $r$).

* Mas, além de uma direção, um vetor também um [sentido]{.hl}.

* Na @fig-retas, o vetor $\vetor w = -\ve1 - 2\ve2$ tem a mesma direção da reta $r$, mas seu sentido é oposto ao sentido do vetor $\vetor v$.

* Então, [qual dos dois vetores $\vetor v$ e $\vetor w$ representa a reta $r$?]{.hl}

* Vamos decidir esta questão do seguinte modo: [nossas retas também vão ter um sentido]{.hl}. Ou seja, vamos trabalhar com [retas orientadas]{.hl}.

* Na @fig-retas, então, os vetores $\vetor v$ e $\vetor w$ representam duas retas $r$ e $r'$, ambas com a mesma direção, mas com sentidos opostos.

* Mas, além de direção e sentido, um vetor também tem um [comprimento]{.hl} (ou [magnitude]{.hl}, ou [módulo]{.hl}, ou [norma]{.hl}).

* Na @fig-vetores, os $3$ vetores $\vetor v_1, \vetor v_2$ e $\vetor v_3$ têm a mesma direção e sentido que a reta $r$.

  ![Vetores de magnitudes diferentes](figures/reta-vetores.png){width=80% fig-alt="Retas e vetores paralelos, com módulos diferentes." #fig-vetores}

* De novo, vamos combinar que [cada um destes vetores define uma reta diferente]{.hl}, todas as retas com a mesma direção e sentido, mas [cada reta com uma magnitude (ou peso) diferente]{.hl}.

* Você pode imaginar o peso de uma reta como a [velocidade]{.hl} com que um ponto percorre a reta, ou como a [velocidade]{.hl} com que a reta avança na direção e no sentido especificados pelo vetor.


  ::: {.callout-note}
  
  ### Resumindo: vetores $=$ retas homogêneas orientadas e com peso
  
  Um vetor 
  $$
  \vetor v = a\ve1 + b\ve2
  $$
  (com $a, b \in \reais$, e com pelo menos um dentre $a$ e $b$ diferente de zero) [representa uma reta homogênea orientada, com a direção e o sentido de $\vetor v$, e com peso igual à norma de $\vetor v$:]{.hl} 
  $$
  ||\vetor v|| = \sqrt{a^2 + b^2}
  $$
  
  :::


## Vetores e retas em $\reais^3$ 

* Agora, vamos trabalhar em $\reais^3$.

* Tudo que falamos acima sobre vetores e retas em $\reais^2$ se aplica a vetores e retas em $\reais^3$, com as seguintes alterações:

  * A base canônica agora é $\{ \ve1, \ve2, \ve3 \}$, onde os vetores correspondem aos eixos $x$, $y$ e $z$, respectivamente.
  
  * Logo, um vetor em $\reais^3$ é escrito como $\vetor v = x\ve1 + y\ve2 + z\ve3$, com $x, y, z \in \reais$.
  
  * Cada vetor $\vetor v = a\ve1 + b\ve2 + c\ve3$ (com $a, b, c \in \reais$, e com pelo menos um dentre $a$, $b$ e $c$ diferente de zero) representa uma reta homogênea orientada, com a direção e o sentido de $\vetor v$, e com peso igual à norma de $\vetor v$:
    $$
    ||\vetor v|| = \sqrt{a^2 + b^2 + c^2}
    $$

* A @fig-reta-r3 mostra um exemplo.

  ![Vetor e reta em $\reais^3$](figures/reta-r3.png){width=100% fig-alt="Reta e vetor em R3." #fig-reta-r3}


## Bivetores e planos em $\reais^3$

* Agora, ainda em $\reais^3$, considere planos [homogêneos]{.hl} (que contêm a origem).

* Um plano homogêneo neste espaço tridimensional é definido por dois vetores linearmente independentes (isto é, com direções diferentes).

* Por exemplo, o plano da @fig-plano é definido pelos vetores $\vetor v = \ve1 + 2\ve2 + \ve3$ e $\vetor w = 3\ve1 - \ve2$.

  ![Plano homogêneo em $\reais^3$](figures/plano.jpg){width=100% fig-alt="Plano homogêneo em R3." #fig-plano}

* De novo, como fizemos antes com as retas, [vamos querer levar em conta os sentidos e as normas]{.hl} destes dois vetores na maneira como eles definem um plano.

* Quanto aos sentidos: vamos dizer que o plano gerado por estes dois vetores pode ter [duas orientações]{.hl}, dependendo da [ordem em que tomarmos os vetores]{.hl}.

* Detalhando: o plano gerado por $\vetor v$ e $\vetor w$ (nesta ordem) vai ter uma orientação, e o plano gerado por $\vetor w$ e $\vetor v$ (nesta ordem) vai ter a orientação oposta.

* Se você revisou os [tópicos recomendados sobre vetores](#topicos-vetores), você deve estar lembrando que o [produto vetorial]{.hl} tem um comportamento parecido: 

* Quando $\vetor v$ e $\vetor w$ são linearmente independentes, o resultado do produto vetorial $\vetor v \times \vetor w$ é um vetor [perpendicular]{.hl} ao plano definido por $\vetor v$ e $\vetor w$.

* O resultado do produto vetorial $\vetor v \times \vetor w$ é um vetor que tem o [sentido oposto]{.hl} ao resultado do produto vetorial $\vetor w \times \vetor v$. O sentido de cada resultado é dado pela regra da mão direita.

* Além disso, a [norma]{.hl} do produto vetorial $\vetor v \times \vetor w$ --- que é igual à norma do produto vetorial $\vetor w \times \vetor v$ --- tem o mesmo valor do que a [área do paralelogramo]{.hl} definido por $\vetor v$ e $\vetor w$. Veja a @fig-prod-vet.
  
  ![Produtos vetoriais](figures/prod-vet.jpg){width=100% fig-alt="Produtos vetoriais." #fig-prod-vet}
  
* Com isto, temos tudo de que precisamos para definir planos homogêneos que, além de direção, têm [peso]{.hl} (magnitude) e [orientação]{.hl} (sentido):

* O [peso]{.hl} do plano definido por $\vetor v$ e $\vetor w$ tem o mesmo valor absoluto da [área do paralelogramo ]{.hl}, com o sinal positivo ou negativo, dependendo da regra da mão direita. Na @fig-prod-vet, o peso do plano definido por $\vetor v$ e $\vetor w$ (nesta ordem) é [negativo]{.hl}, e o peso do plano definido por $\vetor w$ e $\vetor v$ (nesta ordem) é [positivo]{.hl}.

* A [orientação]{.hl} do plano definido por $\vetor v$ e $\vetor w$ (nesta ordem) é oposta à orientação do plano definido por $\vetor w$ e $\vetor v$ (nesta ordem). 

* [Vamos chamar de bivetor]{.hl} este plano homogêneo, orientado e com peso.

* A @fig-bivetor-wv mostra o bivetor definido por $\vetor w$ e $\vetor v$ (nesta ordem). A orientação é dada por uma seta circular.

  ![Bivetor definido por $\vetor w$ e $\vetor v$ (nesta ordem)](figures/bivetor-wv.jpg){width=100% fig-alt="Bivetor w e v." #fig-bivetor-wv}

* A @fig-bivetor-vw mostra o bivetor definido por $\vetor v$ e $\vetor w$ (nesta ordem). A orientação é oposta à do bivetor na @fig-bivetor-wv.

  ![Bivetor definido por $\vetor v$ e $\vetor w$ (nesta ordem)](figures/bivetor-vw.jpg){width=100% fig-alt="Bivetor v e w." #fig-bivetor-vw}
  
* Nas figuras, representamos um bivetor como uma [área orientada]{.hl} em um plano.

* Mas [a forma desta área não é importante]{.hl}. As figuras mostram paralelogramos, mas os mesmos bivetores poderiam ser mostrados como círculos, triângulos etc. com a mesma área.

* As figuras parecem diferenciar planos (que são infinitos) e bivetores (que têm, associados a eles, áreas finitas). Mais adiante, vamos ver que, em algumas aplicações, [podemos interpretar um bivetor como representando o plano no qual ele está contido]{.hl}; em outras aplicações, [podemos interpretar um bivetor como uma porção finita do plano]{.hl}.


## O produto externo de vetores cria bivetores

* Agora podemos definir a operação que cria um bivetor a partir de dois vetores: [o produto externo, representado pelo símbolo $\wedge$]{.hl}.

* O bivetor da @fig-bivetor-wv é o resultado de $\vetor w \wedge \vetor v$.

* O bivetor da @fig-bivetor-vw é o resultado de $\vetor v \wedge \vetor w$.

* Nos exemplos da seção anterior, vimos que [a orientação e o peso do bivetor mudam de acordo com a ordem dos vetores]{.hl}. Então,

  $$
  \vetor v \wedge \vetor w = -(\vetor w \wedge \vetor v)
  $$

* Esta propriedade é chamada [antissimetria]{.hl}.

* A antissimetria tem uma consequência interessante: [o produto externo entre um vetor e ele mesmo é o bivetor nulo]{.hl}:

  $$
  \vetor v \wedge \vetor v = -(\vetor v \wedge \vetor v) \iff \vetor v \wedge \vetor v = \vetor 0
  $$

* No produto $\vetor v \wedge \vetor w$, [se multiplicarmos um dos vetores por um escalar $\lambda$]{.hl}, [o peso]{.hl} (isto é, a área) do bivetor resultante [também será multiplicado por $\lambda$]{.hl}. Então,

  $$
  \lambda (\vetor v \wedge \vetor w) = \lambda\vetor v \wedge \vetor w = \vetor v \wedge \lambda\vetor w
  $$

* Esta propriedade é chamada [bilinearidade]{.hl}.

* A bilinearidade nos permite escrever $\lambda (\vetor v \wedge \vetor w)$ sem os parênteses. Então, a antissimetria pode ser escrita como

  $$
  \vetor v \wedge \vetor w = -\vetor w \wedge \vetor v
  $$

* Uma consequência da bilinearidade é que [o produto externo de vetores paralelos é nulo]{.hl}. Se $\vetor w = \lambda \vetor v$, então

  \begin{align*}
    \vetor v \wedge \vetor w 
    &= \vetor v \wedge \lambda \vetor v \\
    &= \lambda\vetor v \wedge \vetor v \\
    &= \lambda \vetor 0 \\
    &= \vetor 0
  \end{align*}

* Finalmente, o produto externo distribui sobre a adição de vetores:

  \begin{align*}
    \vetor v \wedge (\vetor w + \vetor u) 
    &= (\vetor v \wedge \vetor w) + (\vetor v \wedge \vetor u) \\
    (\vetor v + \vetor w) \wedge \vetor u 
    &= (\vetor v \wedge \vetor u) + (\vetor w \wedge \vetor u)
  \end{align*}

* ??? Guardar este exemplo para depois ??? Para um [exemplo numérico]{.hl}, considere os vetores da @fig-plano:

  \begin{align*}
    \vetor v &= \ve1 + 2\ve2 + \ve3 \\
    \vetor w &= 3\ve1 - \ve2
  \end{align*}

  O produto externo $\vetor v \wedge \vetor w$ será
  
  \begin{align}
    \vetor v \wedge \vetor w 
    &= (\ve1 + 2\ve2 + \ve3) \wedge (3\ve1 - \ve2) \\
    &= ((\ve1 + 2\ve2 + \ve3) \wedge 3\ve1) - ((\ve1 + 2\ve2 + \ve3) \wedge \ve2) \\
    &= \underbrace{\ve1 \wedge 3\ve1}_{\vetor 0}
    + 2\ve2 \wedge 3\ve1 
    + \ve3 \wedge 3\ve1 
    - (\ve1 \wedge \ve2 
      + \underbrace{2\ve2 \wedge \ve2}_{\vetor 0} 
      + \ve3 \wedge \ve2) \\
    &= 2\ve2 \wedge 3\ve1 
    + \ve3 \wedge 3\ve1 
    - \ve1 \wedge \ve2 
    - \ve3 \wedge \ve2 \\
    &= 6\ve2 \wedge \ve1 
    + 3\ve3 \wedge \ve1 
    - \ve1 \wedge \ve2 
    - \ve3 \wedge \ve2 \\
    &= -6\ve1 \wedge \ve2 
    + 3\ve3 \wedge \ve1 
    - \ve1 \wedge \ve2 
    + \ve2 \wedge \ve3 \\
    &= -7\ve1 \wedge \ve2 
    + \ve2 \wedge \ve3
    + 3\ve3 \wedge \ve1 
  \end{align}
  
  Observe que
  
  * A antepenúltima linha é o resultado de aplicar a bilinearidade à linha anterior, multiplicando os coeficientes dos vetores de um mesmo produto externo.
  * A penúltima linha é o resultado de aplicar a antissimetria, mudando o sinal e a ordem dos fatores de $6\ve2 \wedge \ve1$.
  * A última linha é o resultado de aplicar a distributividade.

??? Exercícios???


## Soma de bivetores


## Trivetores em $\reais^3$


## $k$-vetores em $\reais^n$


## Resolvendo problemas com $\wedge$


## Representando subespaços homogêneos orientados e com peso


## *Blades*


## Multivetores


## Resumo


## Exercícios


